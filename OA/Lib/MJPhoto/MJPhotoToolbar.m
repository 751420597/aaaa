...
- (void)setPhotos:(NSArray *)photos
{
    ...
    if (_photos.count>1)
    {
        _pageCtl = [[UIPageControl alloc]initWithFrame:CGRectMake(([AdaptInterface convertWidthWithWidth:iphone6Width]-100.f)/2,CGRectGetMaxY(self.bounds)-30.f, 100.f, 37.f)];
        _pageCtl.numberOfPages=[_photos count];
        _pageCtl.currentPageIndicatorTintColor=[UIColor whiteColor];
        _pageCtl.pageIndicatorTintColor=[UIColor lightGrayColor];
        _pageCtl.contentVerticalAlignment=UIControlContentVerticalAlignmentCenter;
        _pageCtl.currentPage=_currentPhotoIndex;
