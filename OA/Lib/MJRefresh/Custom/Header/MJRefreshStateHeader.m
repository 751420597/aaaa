...
//    [userDefaults synchronize];
    NSInteger errorCode=[userDefaults integerForKey:@"kCFURLErrorTimedOut"];
    // 重新设置key（重新显示时间） 判断当有网络的时候并且当前状态为正在刷新中。。再去更新时间，没有网络的时候不更新时间，并判断此时网络错误码是不是-1001，不是则更新时间，是，则不更新，不管更不更新都得重置userdefaults保存的数值，方便下次网络请求获取数值
    if ([AdaptInterface isConnected]==YES&& state == MJRefreshStateIdle&&errorCode!=-1001) {
        
        self.lastUpdatedTimeKey = self.lastUpdatedTimeKey;
       
    }
    [userDefaults setInteger:0 forKey:@"kCFURLErrorTimedOut"];
    [userDefaults synchronize];
}
@end
