...
- (instancetype)initWithTitles:(NSArray *)titles{
    ...
    if (self) {
        ...
        if (!_oneLabel) {
            ...
            _oneLabel.lineBreakMode = NSLineBreakByWordWrapping|NSLineBreakByTruncatingTail;
            _oneLabel.numberOfLines = 1;
        ...
        if (!_twoLabel) {
            ...
            _twoLabel.lineBreakMode = NSLineBreakByWordWrapping|NSLineBreakByTruncatingTail;
             _twoLabel.numberOfLines = 1;
...
- (void)beginScroll{
    ...
    //[[NSRunLoop mainRunLoop]addTimer:self.timer forMode:NSDefaultRunLoopMode];
     [[NSRunLoop mainRunLoop]addTimer:self.timer forMode:NSRunLoopCommonModes];//加到这个模式下的 loop 防止跟轮播图冲突
}
